...
fn fixture_server(material: &TlsMaterial, status: &'static str) -> (String, thread::JoinHandle<()>) {
    let certs = rustls_pemfile::certs(&mut material.cert_pem.as_bytes())
        .collect::<Result<Vec<_>, _>>().unwrap();
    let key = rustls_pemfile::private_key(&mut material.key_pem.as_bytes()).unwrap().unwrap();
        ...
        let stream = loop {
            match listener.accept() {
                ...
                Err(err) if err.kind() == std::io::ErrorKind::WouldBlock => {
                    assert!(Instant::now() < deadline, "fixture did not receive a connection");
...
fn security_http_accepts_only_the_invite_certificate() {
    ...
    let (url, server) = fixture_server(&other, "HTTP/1.1 200 OK\r\nContent-Length: 0\r\nConnection: close\r\n\r\n");
    assert!(client.get(url).send().is_err());
    server.join().unwrap();
    let (url, server) = fixture_server(&pinned, "HTTP/1.1 200 OK\r\nContent-Length: 0\r\nConnection: close\r\n\r\n");
    assert_eq!(client.get(url).send().unwrap().status(), reqwest::StatusCode::OK);
    server.join().unwrap();
}
...
fn security_broker_http_never_follows_credential_redirects() {
    ...
    let response = http_client(Some(&material.cert_pem)).unwrap()
        .post(url).json(&serde_json::json!({"passphrase": "dummy-fixture"})).send().unwrap();
...
fn security_pin_store_is_exclusive_and_rejects_missing_or_multiple_certificates() {
    ...
    for pem in [String::new(), "not a certificate".to_owned(), format!("{}{}", material.cert_pem, material.cert_pem)] {
