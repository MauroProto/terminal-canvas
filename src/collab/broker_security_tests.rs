...
async fn security_pending_guest_cannot_gain_data_access_by_reconnecting() {
    ...
    relay_payload(
        ...
        session_id,
        StreamAuth::Host,
        Message::Binary(b"private-fixture".to_vec()),
    )
    .await;
    relay_payload(
        &state,
        ...
        Message::Binary(b"unapproved-input".to_vec()),
    )
    .await;
    assert!(
        guest_rx.try_recv().is_err(),
        "pending guest received shared data"
    );
...
async fn security_heartbeat_disconnect_preserves_pending_authorization() {
    ...
    let now = Instant::now();
    {
        ...
        session.guests.get_mut(&guest_id).unwrap().last_seen =
            now - HEARTBEAT_TIMEOUT - Duration::from_secs(1);
    }
...
async fn security_session_creation_rejects_unbounded_phc_policy() {
    ...
    let result = create_share_session(
        ...
        Json(CreateShareSessionRequest {
            invite_secret: "fixture-invite".to_owned(),
            invite_expires_at: None,
            passphrase_hash: Some(valid.replace("m=19456", "m=4294967295")),
            trusted_devices: Vec::new(),
        }),
    )
    .await;
...
async fn security_password_work_is_bounded_and_never_holds_the_session_registry() {
    ...
    let body = JoinShareSessionRequest {
        display_name: "Second fixture".to_owned(),
        invite_secret: "fixture-invite".to_owned(),
        device_id: "second".to_owned(),
        passphrase: Some("fixture-passphrase".to_owned()),
    };
    let all_slots = state
        .verification_slots
        .clone()
        .try_acquire_many_owned(MAX_PASSWORD_VERIFICATIONS as u32)
        .unwrap();
    assert_eq!(
        prepare_join_verification(&state, session_id, &body)
            .await
            .err()
            .unwrap()
            .0,
    ...
    assert!(
        ...
        "password work retained the global registry lock"
    );
    assert_eq!(
        state.verification_slots.available_permits(),
        MAX_PASSWORD_VERIFICATIONS - 1
    );
    drop(preparation);
    assert_eq!(
        state.verification_slots.available_permits(),
...
async fn security_join_rechecks_invite_rotation_after_verification() {
    ...
    rotate_invite(
        ...
        Json(RotateInviteRequest {
            host_token,
            invite_secret: "rotated-fixture".to_owned(),
            invite_expires_at: None,
        }),
    )
    .await
    ...
    assert_eq!(
        state.inner.lock().await.sessions[&session_id].guests.len(),
        1
    );
}
