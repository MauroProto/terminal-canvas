...
fn host_fixture(panel_count: usize) -> (CollabManager, GuestId, SharedWorkspaceSnapshot) {
    ...
    let snapshot = SharedWorkspaceSnapshot {
        ...
        panels: (0..panel_count).map(|_| SharedPanelSnapshot {
            panel_id: Uuid::new_v4(), title: "Fixture terminal".to_owned(),
            position: [0.0, 0.0], size: [800.0, 600.0], color: [0, 0, 0],
            z_index: 0, focused: false, minimized: false, alive: true,
            preview_label: String::new(), share_scope: PanelShareScope::Controllable,
            visible_text: "fixture".to_owned(), history_text: String::new(),
            controller: None, controller_name: None, queue_len: 0,
        }).collect(),
    };
    ...
    manager.host = Some(HostSessionContext {
        session_id: ShareSessionId(Uuid::new_v4()), workspace_id: snapshot.workspace_id,
        host_token: "fixture-token".to_owned(), session_secret: random_secret(),
        previous_session_secret: None, invite_secret: "fixture-invite".to_owned(),
        invite_expires_at: None, requires_passphrase: false, tls_cert_pem: String::new(),
        invite_code: String::new(), guests: HashMap::from([(guest_id, GuestPresence {
            id: guest_id, display_name: "Fixture guest".to_owned(), joined_at: Utc::now(),
            connection_state: GuestConnectionState::Connected,
        })]), pending_joins: Vec::new(), pending_control_requests: Vec::new(),
        terminal_controls: HashMap::new(), last_snapshot: Some(snapshot.clone()),
        last_snapshot_sent_at: None, rekey_recovery_until: None,
        last_rekey_recovery_at: None, next_message_seq: 1,
    });
...
fn request(terminal_id: Uuid, guest_id: GuestId) -> ControlRequest {
    ControlRequest {
        terminal_id, guest_id, display_name: "Fixture guest".to_owned(), requested_at: Utc::now(),
    }
}
...
fn security_unknown_control_targets_never_poison_host_snapshots() {
    ...
    assert!(manager.host.as_ref().unwrap().last_snapshot_sent_at.is_some());
}
...
fn security_control_requests_are_bounded_and_cleanup_recovers_capacity() {
    ...
    let host = manager.host.as_ref().unwrap();
    assert_eq!(host.pending_control_requests.len(), MAX_PENDING_CONTROLS_PER_GUEST);
